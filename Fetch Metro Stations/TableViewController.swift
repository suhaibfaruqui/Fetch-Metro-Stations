...
class TableViewController: UIViewController,CLLocationManagerDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    var stationsDistance : NSDictionary = NSDictionary()
    ...
    func locationManager(manager: CLLocationManager, didFailWithError error: NSError) {
        let errorAlert: UIAlertView = UIAlertView(title: "Error", message: "Failed to Get Your Location", delegate: nil, cancelButtonTitle: "OK", otherButtonTitles: "")
        errorAlert.show()
    }
    ...
    func fetchMetroStations() {
        ...
        let stationNameIconData: NSData? = NSData(contentsOfURL: NSURL(string: "https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=AIzaSyD4dwegCF0EPI2iwI7jrnOmPohugtYpyxU&location=\(myLocation.latitude),\(myLocation.longitude)&radius=5000&type=subway_station")!)!
        ...
        var url : String = "https://maps.googleapis.com/maps/api/distancematrix/json?mode=driving&language=en&key=AIzaSyD4dwegCF0EPI2iwI7jrnOmPohugtYpyxU&origins=\(myLocation.latitude),\(myLocation.longitude)&destinations="
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.drivingDistance.text = "Driving Distance is : \((stationsDistance.objectForKey("rows")!.objectAtIndex(0).objectForKey("elements")!.objectAtIndex(indexPath.row).objectForKey("distance")!.objectForKey("text")!) as! String)"
        cell.address.text = "Station Address is : \((stationsDistance.objectForKey("destination_addresses")!.objectAtIndex(indexPath.row)) as! String)"
